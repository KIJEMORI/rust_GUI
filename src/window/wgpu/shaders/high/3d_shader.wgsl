...
fn scene_sdf(world_p: vec3<f32>, start_idx: u32, count: u32) -> f32 {
    var res = 1000.0;
    for (var i = 0u; i < count; i = i + 1u) {
        let inst = instances[start_idx + i];

        let local_p = (inst.transform * vec4<f32>(world_p, 1.0)).xyz;
        ...
        // k = 0.3 или 0.8 для сглаживания (smin)
        res = smin(res, d, 0.3);
    }
    return res;
}
