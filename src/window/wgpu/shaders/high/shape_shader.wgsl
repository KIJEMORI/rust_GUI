...
fn vs_main(model: VertexInput) -> VertexOutput {
    ...
    let scroll_offset = scroll_buffer.offsets[model.scroll_id].xy;
    let final_pos = model.position + scroll_offset;
    ...
    out.p_a = model.p_a + scroll_offset;
    out.p_b = model.p_b;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (in.params.y < 0.5) { // Прямоугольник
