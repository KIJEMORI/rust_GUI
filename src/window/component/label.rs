...
impl Drawable for Label {
    fn print(
        ...
        if self.panel.base.visible_on_this_frame {
            ...
            let mut pad_rect = self.panel.layout.padding_area(&rect);
            x1 = pad_rect.x1 + x1;
            y1 = pad_rect.y1 + y1;

            pad_rect.set_position(x1, y1);

            if self.select_index_start != self.select_index_end {
                let mut select_rect = self.select_rect.clone();
                select_rect.set_position(select_rect.x1, y1);
            ...
            if self.cursor_need {
                ...
                cursor_rect.set_position(cursor_rect.x1, pad_rect.y1);
    ...
    fn resize_one(&mut self, ctx: &LayoutContext) {
        ...
        let rect = self.panel.base.rect.clone();
        let mut pad_rect = self.panel.layout.padding_area(&rect);

        let width = pad_rect.max.get_width();
        let height = pad_rect.max.get_height();

        let scale_x = width as f32 / rect.max.get_width() as f32;
        let scale_y = height as f32 / rect.max.get_height() as f32;

        self.scale = self.max_scale * scale_x.min(scale_y);

        pad_rect.set_position(rect.x1 + area.x1, rect.y1 + area.y1);
        ...
        if let Some(scroll) = &mut self.panel.scroll {
            ...
            scroll.set_slider_height_width(pad_rect.min.get_height(), pad_rect.min.get_width());
        }
        ...
        if self.select_index_start != self.select_index_end {
            ...
            x2_offset = x2_offset.min(pad_rect.get_x2());

            let first_point = ((pad_rect.x1 as f32 + x1_offset.min(x2_offset)), pad_rect.y1);
            let second_point = (
                (pad_rect.x1 as f32 + x1_offset.max(x2_offset)),
                pad_rect.get_y2(),
            );
        ...
        let mut x_pos = pad_rect.x1 as f32 + x_offset;

        let x2 = pad_rect.get_x2();
            ...
            && self.change_cursor
        {
            ...
            } else if x_pos < pad_rect.x1 - scroll.offset.0 {
                let scroll_x_offset = pad_rect.x1 - scroll.offset.0 - x_pos;
        ...
        self.cursor = Area::new_from_coord((x_pos, pad_rect.y1), (x_pos + 1.0, pad_rect.get_y2()));
    }
...
impl KeyboardControlDrawable for Label {
